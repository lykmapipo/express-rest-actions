...
const getFor = (optns) => {
  // ensure options
  const options = common.mergeObjects(defaultOptions, optns);
  ...
  const httpGet = (request, response, next) => {
    ...
    if (filterParams) {
      mquery.filter = common.mergeObjects(
        lodash.omit(params, ...ignoreParams),
        mquery.filter
      );
    }

    // obtain get options
    const query = common.mergeObjects(mquery, { params });
...
const getByIdFor = (optns) => {
  ...
  const httpGetById = (request, response, next) => {
    ...
    if (filterParams) {
      mquery.filter = common.mergeObjects(
        lodash.omit(extraParams, ...ignoreParams),
        mquery.filter
      );
    }

    // obtain mquery options and id path param
    const query = common.mergeObjects(mquery, { _id: id, id }, { params });
...
const postFor = (optns) => {
  ...
  const httpPost = (request, response, next) => {
    ...
    if (bodyParams) {
      query = common.mergeObjects(lodash.omit(params, ...ignoreParams), body);
    }

    // pack request body with extras
    query = common.mergeObjects(query, { params });
...
const patchFor = (optns) => {
  ...
  const httpPatch = (request, response, next) => {
    ...
    // extend body with params
    if (bodyParams) {
      query = common.mergeObjects(lodash.omit(extraParams, ...ignoreParams), body);
    }

    // extend filter with params
    if (filterParams) {
      query.filter = common.mergeObjects(lodash.omit(extraParams, ...ignoreParams));
    }

    // pack request body and id path param
    query = common.mergeObjects(query, { _id: id, id }, { params });
...
const deleteFor = (optns) => {
  ...
  const httpDelete = (request, response, next) => {
    ...
    const query = common.mergeObjects({ _id: id, id, soft }, { params });
